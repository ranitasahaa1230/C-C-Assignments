...
// let fact=1;
// for(i=1;i<=largestNumFact;i++){
//     fact*=i;
// }
// console.log(fact);
const currFact = numbers.reduce((great, num) => {
  let currentFactorial = Array.from({ length: num })
    .map((_, i) => i + 1)
    .reduce((fact, val) => fact * val, 1);

  return currentFactorial > great ? currentFactorial : great;
});
...
const filterStu = students
  .filter((s) => s.score > 90)
  .reduce((acc, cval, i, arr) => acc + cval.score / arr.length, 0);
...
const filterBooks = books.filter((s) => s.year < 2000).map((acc) => acc.title);
// console.log(filterBooks);

const strings = ["hello world", "i am openai", "welcome to javascript"];
const charAtCapital = strings.map((s) =>
  s
    .split(" ")
    .map((word) => word.charAt(0).toUpperCase() + word.substring(1))
    .join(" ")
);
console.log(charAtCapital);
